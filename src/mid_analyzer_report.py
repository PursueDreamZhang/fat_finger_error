...
function renderEvents(){{eventBody.innerHTML=''; const rows=DATA.events.filter(matches); if(!rows.length){{eventBody.innerHTML='<tr><td colspan="11" class="empty">无符合条件的事件。</td></tr>';return}} rows.forEach(r=>{{const recovery=r.recovery80_5s==null?'无有效观察':(r.recovery80_5s?'5秒恢复≥80%':'5秒未达80%'); const complete=r.mfe_mae_window_complete?'60秒完整':'窗口不足'; eventBody.insertAdjacentHTML('beforeend','<tr><td>'+esc(r.event_id)+'</td><td>'+esc(r.trade_date)+'</td><td>'+esc(r.InstrumentID)+'</td><td>'+esc(r.side==='down'?'向下':'向上')+'</td><td>'+esc(r.mode)+'</td><td>'+num(r.threshold_pct)+'%</td><td>'+esc(r.event_time)+'</td><td>'+num(r.deviation_pct)+'%</td><td>'+num(r.mid_move_pct)+'%</td><td>'+recovery+' / '+complete+'</td><td><button data-event="'+esc(r.event_id)+'">查看窗口</button></td></tr>')}})}}
